...
export class Spider_Max extends Spider {
  totalScraped = 0;
  constructor() {
    super("max");
  }
  async extract(url: string): Promise<number> {
    ...
    for (let i = 0; i <= lastPage; i++) {
      ...
      const products = this.transform({
        ...
        globalAttributes: {
          source: this.source,
          website: "https://www.maxfashion.com",
          region: "GCC",
          currency: "AED",
          listingUrl: url,
        },
  ...
  async crawl(): Promise<void> {
    for (let url of pages) {
      for (const color of colors) {
        ...
        this.setFallbacks({ color, gender: tasks.getGender(url), category: tasks.getCategory(url) });
