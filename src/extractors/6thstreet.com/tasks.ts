...
async function extractProductDetails(page: Page): Promise<ProductRawData | -1> {
  return promiseWithRetry(
    () => {
      return page.evaluate(() => {
         ...
         * Extractors END
         */
        return {} as ProductRawData;
      });
    },
    {
      failCallback: (_error, retries) => {
        console.log(`❗ Failed to extract product details. Attempt: ${retries}. Retrying...`);
      },
    }
  );
}
  ...
  return promiseWithRetry(
    () => {
      return page.evaluate((totalScraped) => {
        ...
        return products.slice(totalScraped).reduce((acc: ProductRawData[], product: Element) => {
          try {
            ...
            if (!url) return acc;
            const listingOrder = acc.length + totalScraped;
...
export default { extractProducts, getCategory, getGender, getTotalProducts, loadMore, scroll, extractProductDetails };
